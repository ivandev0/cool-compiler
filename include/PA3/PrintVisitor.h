...
namespace parser {
    ...
    private:
        void IncreaseOffset() { offset_ += 2; }
        void DecreaseOffset() { offset_ -= 2; }

        void PrintLine(const TreeNode &node) {
            auto lineNumber = std::to_string(node.line_number);
            stream_ << std::setw(static_cast<int>(offset_ + 1)) << "#" << lineNumber << std::endl;
        }

        void PrintStr(const std::string &name) {
            stream_ << std::setw(static_cast<int>(offset_ + name.length())) << name << std::endl;
        }

        void PrintType(const TreeNode &node) {
            stream_ << std::setw(static_cast<int>(offset_ + 2)) << ": " << node.result_type << std::endl;
        }

        // TODO these methods can be in handy if we introduce new type - Symbol
        void PrintSymbol(const std::string &symbol) {
            if (symbol.empty()) return;
            stream_ << std::setw(static_cast<int>(offset_ + symbol.length())) << symbol << std::endl;
        }
        ...
        void VisitIdExpression(IdExpression *expr) override {
            ...
            PrintStr("_object");
            IncreaseOffset();
            stream_ << std::setw(static_cast<int>(offset_ + expr->id.length())) << expr->id << std::endl;
            DecreaseOffset();
            PrintType(*expr);
        }
        ...
        void VisitNoExprExpression(NoExprExpression *expr) override {
            PrintLine(*expr);
            stream_ << std::setw(static_cast<int>(offset_ + expr->value.length())) << expr->value << std::endl;
            PrintType(*expr);
        }
